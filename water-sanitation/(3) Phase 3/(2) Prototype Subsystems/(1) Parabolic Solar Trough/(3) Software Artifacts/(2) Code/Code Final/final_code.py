...
# check if full signal comes in from Nano
def check_level():
    global full
...
# main loop
while True:
    check_switch() # every time loop: check inputs, then respond accordingly
    check_button()
    check_level()

    if (full == True): # go to zero if water level full, refresh at prev state once water level decreases
        prev_state = state
        state = 1

    elif (state == 1): # go to zero and stay there
        state = 1
        led.value = False
        steps = 0
        state_2_substate = 0
